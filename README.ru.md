...
<p align="center">
  ...
  Раскладывайте модели по коллекциям и категориям и листайте их: миниатюры, интерактивное 3D-превью,<br>
...
Проекты из слайсера быстро копятся, а в Finder все файлы `.3mf` выглядят одинаково. Открывать каждый в слайсере, только чтобы понять, что внутри, долго. 3MF Viewer показывает всю библиотеку сразу, раскладывает её по категориям (это обычные папки на диске) и открывает даже модели из миллионов треугольников за пару секунд.

<p align="center">
  <img src="docs/screenshots/demo.gif" alt="3MF Viewer в работе: переключение моделей с автовращением" width="100%">
</p>

## Возможности

- **Коллекции с категориями.** Добавьте одну или несколько папок как коллекции. Их подпапки становятся категориями и подкатегориями любой вложенности, они показаны деревом с числом моделей. Категории можно создавать, переименовывать и удалять, модели можно перетаскивать между ними или переносить через **«Переместить в»**. Все изменения делаются в самих папках, поэтому коллекция на диске всегда устроена так же, как в программе, а изменения из Finder сразу видны в программе.
- **Список моделей.** «Все модели» или любая категория (вместе с подкатегориями), поиск, сортировка по имени, дате или размеру. Файлы `.3mf`, перетащенные из Finder на категорию, копируются в её папку.
...
- **Интерактивное 3D-превью** (SceneKit): вращение, зум, панорамирование, сброс вида (⌘0), автовращение как на поворотном столе, каркасный режим, сетка стола.
...
- **Интерфейс на английском и русском.** По умолчанию английский, переключается в настройках (⌘,) или в меню программы → «Язык». Сторонних зависимостей нет.
...
Интерфейс по умолчанию на английском. Чтобы включить русский, откройте **3MF Viewer → Language → Русский** (или настройки, ⌘,) и перезапустите приложение.
